...
Does it have garbage at the end? You likely extracted it from DSK and didn't include the +3DOS File Header with it. Use the following to extract in CP/M mode:
...
Do you miss the line numbers in the output file? As Assembly doesn't make reference to line numbers, these are meant to help editing and listing, something modern text editors don't need. But you can generate the file with the original line numbers with:
...
Does it have garbage at the end? `speccy-conv.py` likely couldn't file header information that indicates where the BASIC lines end and variable definitions begin. You can manually delete this garbage or re-save your program from a ZX Spectrum doing a `CLEAR` before saving. You can also either use the fix shown above if the file was extracted from DSK, or extract the BASIC file's tape header block to a separate file (say `file.header`) and run:

    python3 speccy-conv.py --tapeHeaderFile file.header bas2u file.bas

`speccy-conv.py` will read the `file.header` and be able to ignore the variables section at the end of your BASIC program. Note that there are [**BASIC Program Heuristics**](#feature-basic-program-heuristics) in place so even in the absense of a header it's unlikely that `speccy-conv.py` is confused.
...
The only difference is in how characters with code points `A3h` and `A4h` are translated to Unicode: as tokens `SPECTRUM` and `PLAY` (default 128K) or User Defined Graphics 🆃 and 🆄 (48K).

There is no support to convert Unicode back to Spectrum's Sinclair BASIC.
...
or, on a Mac/Linux you can simply
...
for help:
    ...
    filenameInput         Filename of input file to convert
    filenameOutput        Filename of output file to generate; defaults to the same as input, with an extension added
...
`filenameOutput`:
You can specify an additional filename argument at the ed of the command line. This will be used as he filename of the output file, rather than taking the `filenameInput` and appending a new extension.

`--prependPlus3DosHeader`:
When generating a Spectrum file (action `u2asm`), you may want that a +3DOS File Header is included, so that the file can be inserted into a DSK.

`--useSoftEOF`:
CP/M uses an inline character with code point `1Fh` called Soft-EOF (also known as Ctrl-Z) to indicate the end of a file. If needed, `speccy-conv.py` can both respect that when reading (actions `asm2u` and `bas2u`), or generate that when creating a Spectrum file (action `u2asm`). Note that this character can cause the early end of file conversion if a line number such as 287 (1**1Fh**), 543 (2**1Fh**), 7936-8191 (**1F**00h-**1F**FFh), etc. exist in the source.

Both `--tapeHeaderFile` and `--prependPlus3DosHeader` are mutually exclusive. Both generate a Spectrum BASIC Header that indicates the file as being `CODE 16384`. This allows you to load it into any memory address in the ZX Spectrum, from BASIC, to inspect it. It will also be evident if you don't select a loading start address (i.e., just `LOAD file CODE`), by going to the screen (`16384`) by default, rather than a random memory address that may crash the computer. This is not perfect as files 7kb or more will rewrite sensitive areas of memory after the screen and crash the computer, but at least the user sees visually what happened just before the crash.
...
* The real length of the current file - in the +3DOS and CP/M disks' metadata this is stored as a multiple of 128, not an exact value.
* Information important to properly convert Sinclair BASIC *(offset to prog)*.
* Information important to allow a ZX Spectrum +3 `LOAD` back the file, once converted back (see `--prependPlus3DosHeader` argument).
...
When the original Spectrum files were retrieved from tape (TAP/TZX/etc.), no +3DOS File Header (as described above) is present. But a Spectrum BASIC Header exists as a preceeding independent tape block.

If the user can extract this tape block to a separate file, `speccy-conv.py` can read the data in it:

* Information important to properly convert Sinclair BASIC *(offset to prog)*.
* Information important to allow a ZX Spectrum `LOAD` back the file, once converted back (see `--tapeHeaderFile` argument).
...
* File length matches 17, as expected.
...
The [ZX Spectrum +3 manual](https://zxspectrumvault.github.io/Manuals/Hardware/SpectrumPlus3Manual.html#part24), part 24 "The memory", documents the BASIC format, as it exists in the ZX Spectrum memory, and recorded on a file.
...
Additional [unofficial documentation](https://problemkaputt.de/zxdocs.htm#spectrumcassette) documents the ZX Spectrum tape header. These are called "Spectrum BASIC Headers" because it's how Sinclair BASIC generates and reads them with `SAVE` and `LOAD` commands.
...
Both the `--tapeHeaderFile` file and the [**Automatic +3DOS File Header Detection**](#feature-plus3dos-header) in `speccy-conv.py` provide options to read such header and provide a proper Sinclair BASIC conversion. However, when exporting files from Spectrum to a modern computer, the header may be lost.
...
+3 BASIC Header documentation indicates that a value of 8000h as a line number indicates a special situation (no automatic start, a.k.a. no `LINE`). Additional [unofficial documentation](https://problemkaputt.de/zxdocs.htm#spectrumcassette) indicates that anything between 8000h to FFFFh (i.e., anything with bit 15 set) indicates this special situation.
...
This means `speccy-conv.py` can read the line number from a Sinclair BASIC source line and act in this way:
...
The kind people from [Planeta Sinclair](http://planetasinclair.blogspot.com) helped me recover my old ZX Spectrum +3 disks in January of 2025. In some HiSoft GEN Assembler files, there were two extra bytes at the begining of the file that the latest version of GEN Plus 3 could not understand. I don't know if those bytes were added during the disk recovery process, or recorded there by some old version of GEN that I was using at the time. However, I realised they always matched a 16-bit number with the length of the file.

`speccy-conv.py` will therefore check if the first two bytes recorded store such value, and skip them if this is the case. It's unlikely that this is a false positive, as that's the position for the first 16-bit line number, this being typically 1 or 10 which is unlikely to match the file length.



## Motivation

This project began when the kind people from [Planeta Sinclair](http://planetasinclair.blogspot.com) managed to recover data from my old ZX Spectrum +3 disks, into Spectrum disk image (DSK) files.

From there I extracted some program files. On my Mac they could not be easily read on a text editor, as they hold binary data. I found an online script to convert Sinclair BASIC to plain text, but I couldn't find one for my HiSoft GEN Assembler files. I could find converters for other assemblers, that didn't work on my files, but none for HiSoft's GEN. I wanted to recover some of my own old Z80 Assembly code for posterity.

Opening my files with an hex editor showed that the format was quite simple. I could write my own script.

I hadn't used Python before, but I was inspired how easy it was to run other authors' scripts ([to extract files from DSK files](https://github.com/muckypaws/AmstradDSKExplorer?tab=readme-ov-file), for instance) on my Mac. So I asked ChatGPT for the first sample version of this script. It gave me 60 working lines of code. This inspired me to expand the feature set manually to the over 1100-line script you see now.
